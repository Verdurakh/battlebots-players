...
1. **A name** — what's the bot called? Make sure the player is happy with it, because **once the first match has begun, the name is locked**. Scores are tracked across rounds by `botName`, so renaming mid-tournament forfeits the running score.
...
- **Don't rename the bot after round 1.** Scores are tracked across rounds by `static botName`. Once the first match has begun, treat the name as locked. If the player asks to rename the bot mid-tournament (e.g. during a review session between rounds), warn them they'll lose their running score and confirm before changing it.
